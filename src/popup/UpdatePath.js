...
function UpdatePath({ onComplete, onClickClose, pathDetails }) {
    ...
    const setValues = () => {
        setNameEn(pathDetails.name_en);
        setNameAr(pathDetails.name_ar);
        setDescriptionEn(pathDetails.description_en);
        setDescriptionAr(pathDetails.description_ar);
        setImage('');
        setCategoryId(pathDetails.category.id);
        setPrice(pathDetails.price);
        const ids = pathDetails.courses.map(course => course.id);
        setCourseIds(ids);
        setOption({ value: pathDetails.category.id, label: language == 'ar' ? pathDetails.category.name_ar : pathDetails.category.name_en });
    }
    ...
    const updatePath = async () => {
        ...
        let result = await Fetch(host + `/teacher/paths/${pathDetails.id}/update`, 'POST', formData);
        ...
        <Box sx={{ backgroundColor: theme.palette.background.paper }} className="shadow-lg w-3/5 h-screen rounded-3xl px-4 py-5 overflow-y-scroll none-view-scroll max-sm:w-4/5 max-sm:translate-x-0 max-sm:left-0 relative" dir={language === 'en' ? 'ltr' : "rtl"}>
            ...
            <Box className=''>
                ...
                <Box className='w-full mt-4'>
                    <MultipleSelectChipPagination page={page} perPage={perPage} total={totalPages} setPage={setPage} isLoading={isLoading} title={<FormattedMessage id="courses" />} data={courses} selected={courseIds} onChange={(value) => setCourseIds(value)} />
                </Box>
                ...
                <Box className='py-2 flex justify-between'>
                    <Button onClick={updatePath} variant="contained" className="w-2/5">
                        {
                            sendWait ?
