...
function UpdateFile({ onComplete, onClickClose, fileDetails }) {
    ...
    const setValues = () => {
        setNameEn(fileDetails.content.name_en);
        setNameAr(fileDetails.content.name_ar);
        setThumbnail('');
        setFile('');
    }
    ...
    const updateFile = async () => {
        ...
        let result = await Fetch(host + `/teacher/courses/files/${fileDetails.content.id}/update`, 'POST', formData);
    ...
    return (
        <Box sx={{ backgroundColor: theme.palette.background.paper }} className="shadow-lg w-3/5 h-fit rounded-3xl px-4 py-5 overflow-y-scroll none-view-scroll max-sm:w-4/5 max-sm:translate-x-0 max-sm:left-0 relative" dir={language === 'en' ? 'ltr' : "rtl"}>
            <Typography variant="h5" className="!font-semibold max-sm:!text-xl"><FormattedMessage id='update_file' /></Typography>
            ...
            <Box className='w-32 h-32 mx-auto rounded-full mb-2 relative border-2 max-sm:w-24 max-sm:h-24 max-sm:mx-auto'>
                ...
                {
                    fileDetails?.content?.thumbnail_url ?
                    <img id='image' src={fileDetails.content.thumbnail_url} className='!z-30 w-full h-full rounded-full relative' />
                    :
            ...
            <Box className=''>
                ...
                <Box className='py-2 flex justify-between'>
                    <Button onClick={updateFile} variant="contained" className="w-2/5">
                        {
                            sendWait ?
